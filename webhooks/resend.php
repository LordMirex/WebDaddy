 ...
 * - email.clicked
 * 
 * Webhook URL: https://yourproject.com/webhooks/resend.php
 */
...
switch ($eventType) {
    ...
    case 'email.complained':
        updateResendEmailStatus($emailId, 'complained', $eventData);
        logWebhookEvent('email.complained', $emailId, $recipient, $eventData);
        error_log("⚠️ Resend: Email $emailId marked as spam by $recipient");
        break;
        
    case 'email.bounced':
        $bounceType = $eventData['bounce']['type'] ?? 'unknown';
        updateResendEmailStatus($emailId, 'bounced', $eventData, "Bounce type: $bounceType");
        logWebhookEvent('email.bounced', $emailId, $recipient, $eventData);
        error_log("❌ Resend: Email $emailId bounced for $recipient ($bounceType)");
        break;
        
    case 'email.opened':
        updateResendEmailStatus($emailId, 'opened', $eventData);
        logWebhookEvent('email.opened', $emailId, $recipient, $eventData);
        error_log("👁️ Resend: Email $emailId opened by $recipient");
        break;
        
    case 'email.clicked':
        logWebhookEvent('email.clicked', $emailId, $recipient, $eventData);
        error_log("🔗 Resend: Link clicked in email $emailId by $recipient");
        break;
