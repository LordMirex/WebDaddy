...
// Get related posts from new smart linking system
$relatedPosts = $blog->getRelatedPosts($post['id'], $post['category_id'], 3);
    ...
    <main class="blog-main">
        <article class="blog-article">
            <header class="blog-article-header">
                ...
                <div class="blog-container blog-container-narrow blog-post-hero-content">
                    <!-- Back Navigation -->
                    ...
                    <!-- Category Badge -->
                    <?php if ($category): ?>
                    <a href="<?= blogGetCategoryUrl($category, $affiliateCode) ?>" class="blog-post-category-badge">
                        <?= htmlspecialchars($category['name']) ?>
                    </a>
                    <?php endif; ?>
                    
                    <!-- Main Title -->
                    <h1 class="blog-post-hero-title"><?= htmlspecialchars($post['title']) ?></h1>
                    
                    <!-- Meta Information Row -->
                    <div class="blog-post-hero-meta">
                        <div class="blog-post-author-info">
                            <?php if ($post['author_avatar']): ?>
                            <img src="<?= htmlspecialchars($post['author_avatar']) ?>" alt="<?= htmlspecialchars($post['author_name']) ?>" class="blog-post-author-avatar">
                            <?php else: ?>
                            <div class="blog-post-author-avatar-placeholder">
                                <?= strtoupper(substr($post['author_name'], 0, 1)) ?>
                            </div>
                            <?php endif; ?>
                            <div class="blog-post-author-details">
                                <div class="blog-post-author-name"><?= htmlspecialchars($post['author_name']) ?></div>
                                <div class="blog-post-meta-items">
                        ...
                        <div class="blog-post-share-compact">
                            <a href="https://wa.me/?text=<?= urlencode($post['title'] . ' ' . $canonicalUrl) ?>" class="blog-post-share-icon" target="_blank" rel="noopener" title="Share on WhatsApp">
                            ...
                            <a href="https://twitter.com/intent/tweet?text=<?= urlencode($post['title']) ?>&url=<?= urlencode($canonicalUrl) ?>" class="blog-post-share-icon" target="_blank" rel="noopener" title="Share on X/Twitter">
                                <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
                            </a>
                            <a href="https://www.facebook.com/sharer/sharer.php?u=<?= urlencode($canonicalUrl) ?>" class="blog-post-share-icon" target="_blank" rel="noopener" title="Share on Facebook">
                                <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                            </a>
                            <a href="https://www.linkedin.com/shareArticle?mini=true&url=<?= urlencode($canonicalUrl) ?>&title=<?= urlencode($post['title']) ?>" class="blog-post-share-icon" target="_blank" rel="noopener" title="Share on LinkedIn">
                                <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
                            </a>
                            <button class="blog-post-share-icon" onclick="copyToClipboard('<?= htmlspecialchars($canonicalUrl) ?>')" title="Copy link">
                                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                ...
                <div class="blog-container blog-container-narrow">
                    <div class="blog-inline-cta blog-inline-cta-banner" style="margin: 48px 0 32px; padding: 32px; border-radius: 12px;">
                        <div class="blog-inline-cta-content">
                            <h3 style="font-size: 20px; font-weight: 800; margin: 0 0 12px 0;">Ready to Build Your Online Presence?</h3>
                            <p style="font-size: 15px; margin: 0 0 18px 0;">Explore our premium website templates and launch your business in 24 hours.</p>
                            <a href="/#templates" style="display: inline-block; background: white; color: var(--blog-accent); padding: 11px 26px; border-radius: 6px; text-decoration: none; font-weight: 700; transition: all 0.3s ease; font-size: 14px;">Browse Templates</a>
                        </div>
                    </div>
                </div>
            </header>
