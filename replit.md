...
- **UPDATED: Checkout Page Navigation** - Back button now returns to previous page:
  - Logo and site name match index page styling
  - "Back" button uses `document.referrer` to return to the page user came from
  - "Return to shopping" link also returns to previous page (or `/` as fallback)
  - "Continue Shopping" button in cart drawer also navigates back to previous page
  - Supports navigation from any page (index, blog, about, contact, etc.) and returns there
